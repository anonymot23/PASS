...
class PassAgent(object):
    ...
    def __init__(self, s_0, x_0, q_0, nb_iter = 100, time_step = 0.01, 
                 ...
                 alpha_min = 1, alpha_max = 3, r = 0.5, var = 5,
                 kappa = 0.1, phi = 1, **kwargs):
    ...
    def update(self, v_0, v_0_past, nb_past, gamma= 0.2):
        ...
        for i in range(self.nb_iter):
            ...
            if (nb_past[i, self.state.idx_q] == 0):# state never visited
                v_0[i, self.state.idx_q] = v_0[i, self.state.idx_q] \
                            + gamma * delta
                            
            elif ( delta * v_0_past[i, self.state.idx_q] >0 ):
                self.alpha_curr = min(self.alpha_curr +1, self.alpha_max)
                gamma_move = (1 + self.r * (self.alpha_curr - 1))
                v_0[i, self.state.idx_q] = v_0[i, self.state.idx_q] \
                            + gamma * gamma_move * delta
