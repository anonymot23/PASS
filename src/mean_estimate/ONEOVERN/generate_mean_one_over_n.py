...
from src.mean_estimate.CONSTANT.constant import ConstantMean


class MeanGeneratorOneOverN(object):
    
    def __init__(self, s_val, nb_iter= 100, nb_episode = 100, 
                 window_size = 50, time_step = 0.01, 
                 mu = 1, alpha = 0.1, var = 5, gamma = 0.1,
                 print_metrics = True,
                 pctg_min = 0.1, **kwargs):
        ...
        self.constMean = ConstantMean(self.s_val, self.nb_iter, self.time_step, 
                                 self.mu, self.alpha, self.var)
        
        
    def initialize_parameters(self):
        # initialize primary parameters
        self.h_0 = 100*np.ones(self.nb_iter)
        self.h_0_past = np.zeros(self.nb_iter + 1)
    ...
    def get_mean(self, h_theo):
        ...
        for ep in range(self.nb_episode):
            ...
            self.h_0, self.h_0_past = self.constMean.update(self.h_0, self.h_0_past,
                                                            gamma = gamma_inner)
            error_val = self.constMean.getLoss(self.h_0, h_theo)
    ...
    def update_gamma(self, ep):
        if ((ep % self.window_size)==self.window_size-1 and (ep> 0)):
            ...
            if (pctg_diff <= self.pctg_min) and (self.cnt_period >= 1):
                ...
                if (self.cnt_reset >= 3):
                    self.gamma = max(self.gamma/2,0.01) # count_gamma_0 = 1
                    self.cnt_reset = 0
            else : 
