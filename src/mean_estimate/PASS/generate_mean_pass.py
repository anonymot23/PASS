...
class MeanGeneratorPass(object):
    ...
    def __init__(self, s_val, nb_iter= 100, nb_episode = 100, 
                 ...
                 mu = 1, alpha = 0.1, alpha_min = 1, alpha_max = 3,
                 var = 5, gamma = 0.1,
                 print_metrics = True,
                 pctg_min = 0.1, **kwargs):
        ...
        self.passMean = PassMean(self.s_val, self.nb_iter, self.time_step, 
                                 self.mu, self.alpha, self.var, self.alpha_min, 
                                 self.alpha_max)
    ...
    def get_mean(self, h_theo):
        ...
        for ep in range(self.nb_episode):
            # Update mean, error
            self.h_0, self.h_0_past = self.passMean.update(self.h_0, self.h_0_past,
                                                           self.nb_past,
                                                           gamma = self.gamma)
            error_val = self.passMean.getLoss(self.h_0, h_theo)
        ...
        if ((ep % self.window_size)==self.window_size-1 and (ep> 0)):
            ...
            if (pctg_diff <= self.pctg_min) and (self.cnt_period >= 1):
                ...
                if (self.cnt_reset >= 3):
                    ...
                    self.cnt_reset = 0
                self.passMean.reset_alpha_curr()
            else : 
...
if __name__ == "__main__":
    ...
    meanGen = MeanGeneratorPass(s_val, nb_iter, nb_episode, window_size,
                                time_step, mu, alpha, alpha_min, alpha_max,
                                var, gamma, print_metrics, pctg_min)
