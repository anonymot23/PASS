...
class SagaAgent(object):
    ...
    def update(self, h_0_mkt, h_0_stay, h_0, h_0_past, nb_past, gamma= 0.2):
        ...
        for i in range(self.nb_iter):
            ...
            # update h_0s
            h_0_mkt , h_0_past = self.update_h(h_0_mkt, h_0_past, nb_past, delta_mkt, 0, gamma)
            h_0_stay, h_0_past = self.update_h(h_0_stay, h_0_past, nb_past, delta_mkt, 1, gamma)
            h_0, h_0_past = self.update_h(h_0, h_0_past, nb_past, delta_mkt, 2, gamma)
    ...
    def update_h(self, h_0, h_0_past, nb_past, delta, idx, gamma): 
        ...
        if j == 0:
            ...
            avg_val = (h_0_past[self.state.q, self.state.pos + 1, :j] * self.exp_mean[:j]).sum() / self.exp_mean[:j].sum()
            h_0[self.state.q, self.state.pos + 1] += gamma * \
                (delta - h_0_past[self.state.q, self.state.pos + 1, r]  + avg_val)
            h_0_past[self.state.q, self.state.pos + 1, idx, r] = delta
                        
        else:
            r = np.random.randint(0, self.n_max)
            avg_val = (h_0_past[self.state.q, self.state.pos + 1, :] * self.exp_mean[:]).sum() / self.exp_mean[:].sum()
            h_0[self.state.q, self.state.pos + 1] += gamma * \
                (delta - h_0_past[self.state.q, self.state.pos + 1, r]  + avg_val)
            h_0_past[self.state.q, self.state.pos + 1, idx, r] = delta
